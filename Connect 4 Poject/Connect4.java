/** Connect4.java: 
  ...
  * @version updated for AP CS Spring 2015, 5/12/15
  */
import java.awt.*;
...
public class Connect4 {
    ...
    private static boolean checkWin(int player) {    // 1 = player, 10 = machine
      ...
      {
        for(int col = 0; col < slot[0].length - 3; col++)
        {
          if(slot[row][col] == player && slot[row][col+1] == player 
               && slot[row][col+2] == player && slot[row][col+3] == player)
          {
            for(int k = 0; k < 4; ++k) {
            slotButton[row][col+k].setBackground(win[player]); 
            slotButton[row][col+k].setOpaque(true); 
            }
            return true;
          }
        }
      ...
      {
        for(int row = 0; row < slot.length - 3; row++)
        {
          if(slot[row][col] == player && slot[row+1][col] == player 
               && slot[row+2][col] == player && slot[row+3][col] == player)
          {
            for(int k = 0; k < 4; ++k) {
              slotButton[row+k][col].setBackground(win[player]); 
              slotButton[row+k][col].setOpaque(true); 
            }
            return true;
          }
        }
      ...
      for(int row = 0; row < slot.length - 3; row++)
      {
        for(int col = 0; col < slot[0].length - 3; col++)
        {
          if(slot[row][col] == player && slot[row+1][col+1] == player 
               && slot[row+2][col+2] == player && slot[row+3][col+3] == player)
          {
            for(int k = 0; k < 4; ++k) {
              slotButton[row+k][col+k].setBackground(win[player]); 
              slotButton[row+k][col+k].setOpaque(true); 
            }
            return true;
          }
        }
      ...
      for(int row = slot.length - 1; row > 3; row--)
      {
        for(int col = 0; col < slot[0].length - 3; col++)
        {
          if(slot[row][col] == player && slot[row-1][col+1] == player 
